...
A common scenario is passing touch through directly to the content, bypassing the AT entirely. Touch ATs rely on the users ability to interact with a series of gestures to control focus, interract, and speak descriptions of on screen elements.  If the content overrides these gestures, users dependent upon these mechanisms for content interaction will be unable to perform basic functions. While overriding assistive technology gestures may be required for certain types of content, if such content covers a significant area of the screen, it could completely block the use of the device by touch AT users, as performing swipe gestures on small patches of screen can be difficult.  
...
Content Warning: Advance warning before loading a control that bypasses the AT warns the user . EX: A warning alert on a control that loads a "paint" window, or a text warning displayed before the control (and tied to control e.g. via aria-describedby)
