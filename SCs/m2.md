...
1. Locate all touch targets/actionable items and identify if the inputs are defined for coarse or fine pointing accuracy
...
I think we need an exception for touch targets that are inline text links IF  WCAG 2.1 is not going to require a minimum text size / minimum line width. For authors using inline text at smaller font sizes, the requirement for a touch target that is 48px high (if the author has to assume that content may be operated via touch, which will be the case for all but content except intranet stuff in controlled environments) is difficult to meet as it may often overlap the preceding or following line of text which may also have inline links. I think asking for 48 is maybe not realistic. Optiobs may be asking for half the width? 
